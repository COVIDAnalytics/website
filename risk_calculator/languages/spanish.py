...
class Spanish(CalcLanguage):
    submit = "Enviar"
    missingFeatureTxt = "La característica que falta, {}, fue calculado como {}."
    hasLabValues = "¿Tienes valores de laboratorio?"
    notEnoughValues = "Por favor inserte al menos {} valores numericos."
    outOfRangeValues = "Por favor inserte un valor numérico para {} entre {} y {}"
    hasO2Value = "¿Tiene el valor para SpO2 o SaO2?"

    def prompt_missing_feature(self, feature):
        return "Por favor inserte un valor para {}.".format(self.get_feature_names()[feature])

    def get_yes(self, yes=True):
        return "Si" if yes else "No"

    def get_gender(self, male=True):
        return "Hombre" if male else "Mujer"
    ...
    def get_page_desc_mortality(self, labs_auc, no_labs_auc):
        return [
            ...
            html.Hr(),
            dcc.Markdown(
                """Los pacientes con COVID-19 en situación grave requieren los recursos de atención médica \
                 más escasos, respiradores y camas de UCIs. Cuando el número de pacientes excede la \
            ...
            dcc.Markdown(
                """* Una calculadora que utiliza datos demográficos, signos vitales y comorbilidades, \
                 ...
                 del triaje cuando llega al hospital. El área bajo la curva (AUC) en \
                 predicciones fuera de muestra es {}.""".format(no_labs_auc),
            ),
            dcc.Markdown(
                """ **NOTA (¡Esta es una versión de desarrollo!):** La precisión de los modelos depende \
                 de la calidad de los datos con los que han sido creados. Lanzaremos nuevas versiones \
                 de la calculadora \
                 a medida que aumente la cantidad de datos que recibamos de nuestras instituciones asociadas. Si \
    ...
    def get_page_desc_infection(self):
        return [
            ...
            dcc.Markdown(
                """Las pruebas para detectar el COVID-19 requieren mucho tiempo, son caras y requieren que \
                 los pacientes \
    ...
    def get_visual_1(self):
        return """El siguiente [gráfico de SHAP](https://github.com/slundberg/shap) resume como cada característica \
            ...
            mientras las características en rojo las aumentan. La contribución de cada característica es proporcional \
            al ancho de la barra. Las barras más anchas tienen mayor importancia en el puntaje de riesgo final. \
    ...
    def get_model_desc_mortality(self, auc, pop, pos):
        return [
            ...
            dcc.Markdown(
                """Nuestro modelo ha sido creado usando datos de {} pacientes (de los cuales {}% fallecidos) \
                 hospitalizados debido a COVID-19 en:""".format(pop[0], str(int(float(pos[0]) * 100))),
            ),
            dcc.Markdown(
                """* La ciudad italiana de Cremona ([Azienda Socio-Sanitaria Territoriale di Cremona]\
                 (https://www.asst-cremona.it/en/home)). Cremona es una de las provincias italianas más afectadas en \
                 Lombardía con varios miles de casos positivos hasta la fecha.""",
            ),
            dcc.Markdown(
                """* [HM Hospitals](https://www.fundacionhm.com/), un grupo hospitalario líder en España con 15 \
                 hospitales generales y 21 centros clínicos que cubren las regiones de Madrid, Galicia y León. """,
            ),
            dcc.Markdown(
                """* [Hartford HealthCare](https://hartfordhealthcare.org), una importante red de \
                hospitales que atiende a pacientes en todo Connecticut (EE. UU.). """,
            ),
            dcc.Markdown(
                """ Dada la demografía de nuestra base de datos, consideramos que nuestro \
                modelo tiene mayor relevancia \
                 en: (a) la población occidental; (b) pacientes severos a agudos; (c) hospitales congestionados. """,
            ),
            html.Hr(),
            html.Div([
                "La calculadora se basa en ", html.A("el clasificador XGBoost.",
                                                     href="https://xgboost.readthedocs.io/"),
                html.Br(),
                "El área bajo la curva (AUC) para predicciones fuera de muestra con {} pacientes \
                (de cuales {}% fueron falleciron) es ".format(pop[1], str(int(float(pos[1]) * 100))),
                html.Span(' {}'.format(auc), style={'color': '#800020', "fontWeight": "bold"}),
                ".",
                html.Br(),
                "Si el usuario no dispone de información sobre todas las variables, nuestra calculadora \
                les imputará un valor e informará al usuario del mismo."
            ]),
            html.Br(),
            dcc.Markdown(
                ...
                El gráfico muestra, para cada característica, el impacto de ésta en el modelo. Para ello, todas las \
                observaciones de dicha característica se distribuyen horizontalmente en función de su valor de SHAP. 
                El valor de SHAP cuantifica el impacto de cada observación \
                en el resultado del modelo, siendo los valores indicadores de una mayor probabilidad de resultado \
                positivo (mortalidad o infección en nuestro caso) y viceversa. Asimismo, \
                cada observación se colorea en función del valor relativo de dicha observación dentro del conjunto \
                de la característica, siendo el azul asignado al valor más bajo y el rojo al más alto. \
                Tomando como ejemplo la edad, se puede observar que el color varía de azul a rojo a medida \
                que el valor SHAP aumenta. Esto indíca que el riesgo de mortalidad o infección ese mayor \
                en la población de mayor edad. En cambio, la evolución del color en el caso de la \
                saturación de oxígeno es a la inversa, lo que indica que \
    ...
    def get_model_desc_infection(self, auc, pop, pos):
        return [
            ...
            html.Hr(),
            html.Div([
                "La calculadora se basa en", html.A("el clasificador XGBoost.", href="https://xgboost.readthedocs.io/"),
                html.Br(),
                "El área bajo la curva (AUC) para predicciones fuera de muestra con {} pacientes \
                (de cuales {}% fueron infectados) es ".format(pop[1], str(int(float(pos[1]) * 100))),
                html.Span(' {}'.format(auc), style={'color': '#800020', "fontWeight": "bold"}),
                ".",
                html.Br(),
            ...
            dcc.Markdown(
                """ Utilizamos [gráficos de SHAP](https://github.com/slundberg/shap) \
                 ...
                 características aparecen ordenadas en orden de importancia decreciente, siendo \
                 la primera la más relevante. \
                 El gráfico muestra, para cada característica, el impacto de ésta en el modelo. Para ello, \
                 todas las observaciones de dicha característica se distribuyen horizontalmente en función de su \
                 valor de SHAP. El valor de SHAP cuantifica el impacto de cada observación \
                 en el resultado del modelo, siendo los valores indicadores de una mayor probabilidad \
                 de resultado positivo (mortalidad o infección en nuestro caso) y viceversa. Asimismo, \
                 cada observación se colorea en función del valor relativo de dicha observación dentro del conjunto \
                 de la característica, siendo el azul asignado al valor más bajo y el rojo al más alto. \
                 Tomando como ejemplo la edad, se puede observar que el color varía de azul a rojo a medida que el \
                 valor SHAP aumenta. Esto indíca que el riesgo de mortalidad o infección ese mayor \
                 en la población de mayor edad. En cambio, la evolución del color en el caso de \
                 la saturación de oxígeno es a la inversa, lo que indica que \
                 son los niveles bajos de saturación los que conllevan mayor riesgo. Nota: el género se \
                 codifica como un valor binario (0 = Masculino, 1 = Femenino), por lo \
                 que los valores "menores" de género corresponden a pacientes masculinos."""
            ),
