...
class Italian(CalcLanguage):
    ...
    def prompt_missing_feature(self, feature):
        return "Inserisci un valore per {}.".format(self.get_feature_names()[feature])
    ...
    def get_page_desc_mortality(self, labs_auc, no_labs_auc):
        return [
            ...
            dcc.Markdown(
                ...
                disponibili in numero esiguo, come ventilatori e letti di terapia intensiva. Quando il numero di \
                pazienti è superiore alla quantità di risorse a disposizione, i medici hanno la pesante responsabilità \
                di scegliere a quali pazienti dare priorità. Per aiutarli nel compiere una decisione fondata su \
                basi matematiche, abbiamo sviluppato un calcolatore di mortalità per \
                pazienti ospidalizzati per COVID-19.
                """,
            ),
            dcc.Markdown(
            ...
            dcc.Markdown(
                """* Un calcolatore che utilizza dati anagrafici, valori vitali, \
                comorbidità e **valori di laboratorio**. \
            ...
            dcc.Markdown(
                ...
                laboratorio**. Nella nostra visione, questo modello verrà usato nella fase di smistamento per un 
                paziente affetto da COVID-19 che va all'ospedale per determinare in modo preliminare la gravità della 
                propria condizione di salute. L'Area Sotto la Curva (AUC) su pazienti fuori dal campione di \
                allenamento è {}.""".format(no_labs_auc),
            ),
            dcc.Markdown(
                """**NOTA (Questa è una versione in fase di sviluppo!):** I modelli sono tanto accurati \
                quanto i dati sui quali vengono allenati. Rilasceremo nuove versioni del calcolatore \
                quando riceveremo ulteriori dati dalle istituzioni con cui collaboriamo. Se una \
                istituzione sanitaria volesse \
    ...
    def get_page_desc_infection(self):
        return [
            ...
            dcc.Markdown(
                """I test per identificare pazienti affetti da COVID-19 impiegano tempo, sono costosi e \
                necessitano che il paziente si presenti di persona al centro specializzato, aumentando la \
                possibilità di essere esposto al virus. Per aiutare nell'identificazione di pazienti sintomatici, \
                abbiamo sviluppato un calcolatore per predirre la probabilità che un paziente sia infetto.""",
            ),
            html.Hr(),
            dcc.Markdown(
                """**NOTA (Questa è una versione in fase di sviluppo!):** I modelli sono tanto accurati quanto \
                i dati sui quali vengono allenati. Rilasceremo nuove versioni del calcolatore quando riceveremo \
                ulteriori dati dalle istituzioni con cui collaboriamo. Se una istituzione sanitaria volesse \
                contribuire al nostro lavoro, cortesemente ci contatti [qui](https://www.covidanalytics.io/contact).""",
            )
        ]
    ...
    def get_results_card_default(self):
        return "I risultati verranno visualizzati qui"

    def get_results_card_infection(self):
        return ["Il rischio di infezione è:", " su 10"]

    def get_visual_1(self):
        return """Il [grafico SHAP](https://github.com/slundberg/shap) sottostante riassume l'effetto che ogni \
            valore ha sulla predizione di rischio. I valori in blu diminuiscono il rischio del paziente, \
            mentre quelli in rosso lo aumentano. L'effetto nella predizione è proporzionale alla larghezza della \
            barra dei valori. Più la barra è larga, maggiore è l'effetto del valore sulla predizione finale di \
            rischio. Nota: il sesso è rappresentato come un valore binario (0=Uomo, 1=Donna)."""

    def get_model_desc_mortality(self, auc, pop, pos):
        return [
            html.H2("Dettagli Tecnici"),
            dcc.Markdown(
                """Il nostro modello è stato allenato su {} pazienti (dei quali {}% deceduti) ospitalizzati per \
                COVID-19 in:""".format(pop[0], str(int(float(pos[0]) * 100))),
            ),
            dcc.Markdown(
            ...
            dcc.Markdown(
                ...
                Nota: il sesso è rappresentato come un valore binario (0=Uomo, 1=Donna), quindi un valore "basso" \
                di esso corrisponde ad un paziente maschio."""),
            dcc.Markdown(
        ...
        return [
            ...
            dcc.Markdown(
                """Utilizziamo [grafici SHAP](https://github.com/slundberg/shap) \
                 ...
                 Quando invece i valori sono orientati da rosso a blu, il rischio diminuisce quando il valore aumenta, \
                 come accade per la saturazione di ossigeno. \
                 Nota: il sesso è rappresentato come un valore binario (0=Uomo, 1=Donna), quindi un valore "basso" di \
                 esso corrisponde ad un paziente maschio."""),
            dcc.Markdown(
                """In generale, l'importanza dei valori inseriti nel calcolatore è la seguente:""",
            )
        ]
